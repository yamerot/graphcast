...
class Loader:
    ...
    def __init__(self, ds, ds_surface, ds_static, 
        ...
        self.forcings_template = xr.Dataset(
            coords={'time': np.arange(0, batchsize * len(target_hours)), 
                    'level': ds.level, 
                    'lat': ds.lat, 
                    'lon': ds.lon}
        )
        ...
        # 4. 数据暂存区
        self.upper_cache = ChunkCache(ds)
        self.surface_cache = ChunkCache(ds_surface)

    def batching(self, ds: xr.Dataset, hours):
        """
        ...
        return xr.concat(moments, dim='time').assign_coords(time=np.array(hours))
    ...
    def update(self, start):
        ...
        targets_datetime = xr.concat(
            [inputs.datetime.sel(time=0) + pd.Timedelta(hours=h) for h in self.target_hours], 
            dim='time')
        ...
        # 将目标时间 (batch 形式) 写入 targets 模板
        targets = self.targets_template.assign_coords(datetime=(('batch', 'time'), forcings.datetime))

        return inputs, targets, forcings

    def get_continuous(self, idx):
        return xr.merge([self.upper_cache.claim(idx), 
                         self.surface_cache.claim(idx), 
                         self.ds_static])
...
def main():
    ...
    os.environ['JAX_COMPILATION_CACHE_DIR'] = 'home/chengzy/graphcast/graphcast/params/tmp/jax_cache'
    ...
    loader = Loader(ds, ds_surface, ds_static, task_config.target_variables, batchsize, '12h', [6, 12, 18, 24])
    ...
    for idx in np.arange(start, end, batchsize):
        ...
        inputs, targets, forcings = loader.update(idx)
